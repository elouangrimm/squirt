...
var readability = {
    ...
    regexps: {
        unlikelyCandidates:
            /ad|aside|combx|comment|community|disqus|extra|foot|header|menu|remark|rss|shoutbox|sidebar|sponsor|ad-break|agegate|pagination|pager|popup|tweet|twitter|figure|figcaption/i,
        ...
        skipFootnoteLink:
            /^\s*(\[?[a-z0-9]{1,2}\]?|^|edit|citation needed)\s*$/i,
    ...
    prepArticle: function (articleContent) {
        ...
        for (var i = articleParagraphs.length - 1; i >= 0; i -= 1) {
            var imgCount =
                articleParagraphs[i].getElementsByTagName("img").length;
            var embedCount =
            ...
            if (
                ...
                embedCount === 0 &&
                objectCount === 0 &&
                articleParagraphs[i].textContent === ""
            ) {
                articleParagraphs[i].parentNode.removeChild(
                    articleParagraphs[i]
                );
            }
        }
    ...
    grabArticle: function () {
        var stripUnlikelyCandidates = readability.flagIsActive(
        ...
        for (nodeIndex = 0; (node = allElements[nodeIndex]); nodeIndex += 1) {
            ...
            if (stripUnlikelyCandidates) {
                ...
                if (
                    ...
                    unlikelyMatchString.search(
                        readability.regexps.okMaybeItsACandidate
                    ) === -1 &&
                    node.tagName !== "BODY"
                ) {
                    remove(node);
            ...
            if (node.tagName === "DIV") {
                if (
                    node.innerHTML.search(
                        readability.regexps.divToPElements
                    ) === -1
                ) {
                    ...
                    try {
                        ...
                        dbg(
                            "Could not alter div to p, probably an IE restriction, reverting back to div.: " +
                                e
                        );
                    }
                } else {
                    /* EXPERIMENTAL */
                    for (
                        var i = 0, il = node.childNodes.length;
                        i < il;
                        i += 1
                    ) {
        ...
        for (var c = 0, cl = candidates.length; c < cl; c += 1) {
            ...
            candidates[c].readability.contentScore =
                candidates[c].readability.contentScore *
                (1 - readability.getLinkDensity(candidates[c]));

            dbg(
                "Candidate: " +
                    candidates[c] +
                    " (" +
                    candidates[c].className +
                    ":" +
                    candidates[c].id +
                    ") with score " +
                    candidates[c].readability.contentScore
            ...
            if (append) {
                ...
                if (
                    ...
                    try {
                        ...
                        dbg(
                            "Could not alter siblingNode to div, probably an IE restriction, reverting back to original."
                        );
        ...
        if (articleContent.textContent.length < 250) {
            ...
            if (readability.flagIsActive(readability.FLAG_STRIP_UNLIKELYS)) {
                ...
                readability.flagIsActive(readability.FLAG_WEIGHT_CLASSES)
            ) {
                readability.removeFlag(readability.FLAG_WEIGHT_CLASSES);
                return readability.grabArticle(page);
            } else if (
                readability.flagIsActive(readability.FLAG_CLEAN_CONDITIONALLY)
            ) {
    ...
    findNextPageLink: function (elem) {
        ...
        for (var i = 0, il = allLinks.length; i < il; i += 1) {
            ...
            if (
                ...
                linkHref in readability.parsedPages
            ) {
                continue;
            }
            ...
            if (
                linkText.match(readability.regexps.extraneous) ||
                linkText.length > 25
            ) {
                continue;
            }
            ...
            if (
                linkData.match(readability.regexps.negative) ||
                linkData.match(readability.regexps.extraneous)
            ) {
                linkObj.score -= 50;
            }
                ...
                success: function (r) {
                    ...
                    if (eTag) {
                        if (eTag in readability.pageETags) {
                            dbg(
                                "Exact duplicate page found via ETag. Aborting."
                            );
                    ...
                    responseHtml = responseHtml.replace(
                        readability.regexps.replaceBrs,
                        "</p><p>"
                    );
                    responseHtml = responseHtml.replace(
                        readability.regexps.replaceFonts,
                        "<$1span>"
                    );
                    ...
                    if (firstP && firstP.innerHTML.length > 100) {
                        for (var i = 1; i <= readability.curPageNum; i += 1) {
                            var rPage = document.getElementById(
                                "readability-page-" + i
                            );
                            if (
                                rPage &&
    ...
    getClassWeight: function (e) {
        ...
        if (typeof e.id === "string" && e.id !== "") {
            if (e.id.search(readability.regexps.negative) !== -1) weight -= 25;

            if (e.id.search(readability.regexps.positive) !== -1) weight += 25;
        }
        return weight;
    },
    ...
    killBreaks: function (e) {
        try {
            e.innerHTML = e.innerHTML.replace(
                readability.regexps.killBreaks,
                "<br />"
            );
        } catch (eBreaks) {
    ...
    clean: function (e, tag) {
        ...
        for (var y = targetList.length - 1; y >= 0; y -= 1) {
            ...
            if (isEmbed) {
                var attributeValues = "";
                for (
                    var i = 0, il = targetList[y].attributes.length;
                    i < il;
                    i += 1
                ) {
        ...
        for (var i = curTagsLength - 1; i >= 0; i -= 1) {
            ...
            var contentScore =
                typeof tagsList[i].readability !== "undefined"
                    ? tagsList[i].readability.contentScore
                    : 0;
            ...
            if (weight + contentScore < 0) {
                ...
                for (var ei = 0, il = embeds.length; ei < il; ei += 1) {
                    if (
                        embeds[ei].src.search(readability.regexps.videos) === -1
                    ) {
                        embedCount += 1;
                    }
                }
    ...
    cleanHeaders: function (e) {
        for (var headerIndex = 1; headerIndex < 3; headerIndex += 1) {
            ...
            for (var i = headers.length - 1; i >= 0; i -= 1) {
                if (
                    readability.getClassWeight(headers[i]) < 0 ||
